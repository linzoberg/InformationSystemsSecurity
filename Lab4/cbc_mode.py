class CBCMode:
    ...
    def __init__(self, block_cipher, block_size=5):
        ...
        Args:
            block_cipher: Объект блочного шифра с методами encrypt_block и decrypt_block
            block_size: Размер блока в байтах (по умолчанию 5)
        """
    ...
    def pad_data(self, data):
        """
        Добавляет padding к данным до размера, кратного block_size.
    ...
    def encrypt(self, data, iv):
        ...
        Args:
            ...
            iv: Начальный вектор (initialization vector) длиной block_size
    ...
    def decrypt(self, encrypted_data, iv):
        ...
        Returns:
            bytes: Дешифрованные данные (без padding)
        """
        if len(iv) != self.block_size:
            raise ValueError(f"IV должен быть длиной {self.block_size} байт")
